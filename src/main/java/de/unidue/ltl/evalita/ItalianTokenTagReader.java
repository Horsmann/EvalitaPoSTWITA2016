...
public class ItalianTokenTagReader
        ...
        throws IOException, CollectionException
    {
        ...
        for (int k = 0; k < sequences.size(); k++) {
            ...
            for (int i = 1; i < sequence.size(); i++) {
                String pairs = sequence.get(i);

                int idxLastSpace = pairs.lastIndexOf("\t");
                String token = pairs.substring(0,
                        (idxLastSpace > -1) ? idxLastSpace : pairs.length());

                String tag = null;
                if (idxLastSpace > -1) {
                    tag = pairs.substring(idxLastSpace + 1);
                }
            ...
            EvalitaId ei = new EvalitaId(aJCas, seqStart, documentText.length());
            ei.setTweetId(sequence.get(0));
            ei.addToIndexes();
